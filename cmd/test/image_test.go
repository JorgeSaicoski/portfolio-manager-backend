...
// Helper function to create test image in database
func CreateTestImage(db interface{}, entityID uint, entityType string, ownerID string) *struct {
	ID           uint
	URL          string
	ThumbnailURL string
	FileName     string
	...
	// This would use the actual Image model
	// For now, returning a simplified struct
	// In real implementation, you'd use gorm to create the record
	return &struct {
		ID           uint
		URL          string
		ThumbnailURL string
